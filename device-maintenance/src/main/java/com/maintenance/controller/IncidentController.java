...
public class IncidentController {
    ...
    public ResponseEntity<ApiResponse<IncidentResponse>> resolve(@PathVariable Long id,
                                                                 @RequestParam(required = false) String resolutionNote,
                                                                 @RequestParam(required = false) Double repairCost) {
        return ResponseEntity.ok(ApiResponse.success(incidentService.resolve(id, resolutionNote, repairCost), "Da giai quyet su co"));
    }
