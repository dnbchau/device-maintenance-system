...
## Cap nhat moi (10/4 2h35)
Backend đã được sửa lỗi và hoàn thiện ở ba phần chính: xử lý dữ liệu ngày giờ, tự động ghi nhận lịch sử sửa chữa và khắc phục lỗi cấu hình khiến hệ thống không khởi động được.

1. **Sửa lỗi tạo repair_history từ API**
   Bổ sung cấu hình JacksonConfig để hỗ trợ nhiều định dạng LocalDateTime, tránh lỗi khi gửi dữ liệu ngày giờ từ client.
   Thêm JsonExceptionHandler để trả về thông báo lỗi rõ ràng khi JSON không hợp lệ hoặc sai định dạng repairDate.
   Viết test (JacksonConfigTest) để kiểm tra việc parse dữ liệu ngày giờ.

2. **Hoàn thiện luồng tự động ghi lịch sử sửa chữa**
   Với Incident
   Khi incident được cập nhật trạng thái RESOLVED hoặc CLOSED:
   Hệ thống tự động tạo bản ghi trong repair_histories.
   Thiết bị chuyển từ BROKEN về ACTIVE (nếu có).
   Dữ liệu ghi nhận gồm:
   device, incident
   performedBy (technician được phân công)
   repairDate từ resolvedAt
   technicianNote từ resolutionNote
   cost mặc định 0.0
   Với Maintenance Schedule
   Khi schedule được cập nhật COMPLETED:
   Hệ thống tự động tạo bản ghi trong repair_histories.
   Thiết bị chuyển về ACTIVE.
   Dữ liệu ghi nhận gồm:
   device, maintenanceSchedule
   performedBy
   repairDate từ actualEndDate
   technicianNote từ notes
   cost từ schedule.cost
   Cải tiến chung
   Tạo helper dùng chung trong RepairHistoryService.
   Thêm kiểm tra chống trùng trong RepairHistoryRepository theo incidentId và maintenanceScheduleId.

3. **Khắc phục lỗi hệ thống không khởi động**
   Xác định nguyên nhân: file application.properties trong target/classes bị rỗng.
   Hệ thống không đọc được cấu hình app.jwt.secret.
   Đã khôi phục lại nội dung cấu hình để ứng dụng khởi động bình thường.
   Kết quả đạt được
   Tự động ghi nhận lịch sử sửa chữa, không cần gọi API thủ công.
   Tránh tạo dữ liệu trùng khi thao tác nhiều lần.
   Xử lý lỗi ngày giờ rõ ràng, dễ debug.
   Ổn định hệ thống, đảm bảo ứng dụng khởi động và hoạt động bình thường.